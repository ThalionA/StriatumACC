...
switch lower(normalisation_to_apply)
    ...
    case 'z-score'
        ...
        data_in = reshape(data_zscored_reshaped, num_neurons, num_bins, n_trials);
    case 'min-max'
        data_reshaped = reshape(data, num_neurons, []);
        ...
        data_in = reshape(data_scaled, num_neurons, num_bins, n_trials);
    otherwise
...
% For the final fitting, default to 100 iterations
if options.maxiters < 100
...
% for iFactor = 1:best_nFactors
%     nexttile
%     shadedErrorBar(1:n_trials, movmean(best_mdl.U{3}(:, iFactor), 10), ...
...
% HSV_early = [H_early, 1, 1];
% colors_early = hsv2rgb(HSV_early);
% 
% HSV_precise = [H_precise, 1, 1];
% colors_precise = hsv2rgb(HSV_precise);
% 
% HSV_random = [H_random, 1, 1];
...
%     nexttile
% 
%     my_simple_errorbar_plot([{best_mdl.U{3}(first_idx, iFactor)}, {best_mdl.U{3}(precise_idx, iFactor)}, {best_mdl.U{3}(random_idx, iFactor)}], [colors_early; colors_precise; colors_random])
%     if iFactor ~= best_nFactors
%         xticks([])
%     end
%     title(sprintf('Variance Explained: %.2f%%', variance_explained(iFactor) * 100))
%     xticklabels({'early', 'precise', 'random'})
% end
% title(t, tca_type)
% 
% end