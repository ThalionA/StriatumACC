function all_data = reorganize_spikes_by_area(all_data, ianimal)
    ...
    % Reorder spikes
    temp_final_spikes = [all_data(ianimal).final_spikes(is_dms, :); all_data(ianimal).final_spikes(is_dls, :); all_data(ianimal).final_spikes(is_acc, :)];
    all_data(ianimal).final_spikes = temp_final_spikes;

    % Reorder areas
    all_data(ianimal).final_areas = [all_data(ianimal).final_areas(is_dms), all_data(ianimal).final_areas(is_dls), all_data(ianimal).final_areas(is_acc)];
end